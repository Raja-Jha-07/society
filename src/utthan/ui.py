...
VERTICAL_SCROLLBAR = "App.Vertical.TScrollbar"
HORIZONTAL_SCROLLBAR = "App.Horizontal.TScrollbar"
...
class ScrollFrame(ttk.Frame):
    def __init__(self, parent: tk.Widget) -> None:
        ...
        scrollbar = ttk.Scrollbar(
            self, orient="vertical", command=canvas.yview, style=VERTICAL_SCROLLBAR,
        )
...
class SocietyApp(tk.Tk):
    ...
    def _configure_styles(self) -> None:
        ...
        style.configure(VERTICAL_SCROLLBAR, width=16, **scrollbar_options)
        style.configure(HORIZONTAL_SCROLLBAR, width=16, **scrollbar_options)
        for scrollbar_style in (VERTICAL_SCROLLBAR, HORIZONTAL_SCROLLBAR):
            style.map(
                scrollbar_style,
                background=[("active", "#627D98"), ("pressed", "#486581")],
    ...
    def tree(self, parent: tk.Widget, columns: list[tuple[str, str, int, str]]) -> ttk.Treeview:
        ...
        vertical = ttk.Scrollbar(
            holder, orient="vertical", command=tree.yview, style=VERTICAL_SCROLLBAR,
        )
        horizontal = ttk.Scrollbar(
            holder, orient="horizontal", command=tree.xview, style=HORIZONTAL_SCROLLBAR,
        )
        ...
        vertical.grid(row=0, column=1, sticky="ns", padx=(4, 0))
        horizontal.grid(row=1, column=0, sticky="ew", pady=(4, 0))
        ...
        tree.bind("<MouseWheel>", lambda event: self._scroll_tree(tree, event, horizontal=False))
        tree.bind("<Shift-MouseWheel>", lambda event: self._scroll_tree(tree, event, horizontal=True))
        return tree

    @staticmethod
    def _scroll_tree(tree: ttk.Treeview, event: tk.Event, horizontal: bool) -> str:
        direction = -1 if event.delta > 0 else 1
        if horizontal:
            tree.xview_scroll(direction * 3, "units")
        else:
